# 엔지니어링 스펙트럼 — 자동차·반도체·로보틱스 산업구조로 읽는 직업의 지도
...
| **역할 스펙트럼 변화** | R&D → 생산관리 → 생산기술 → 설비운영으로 순차 이동 | 단계별로 기능 자체가 다름 (파운드리 내부에도 R&D~생산운영 공존) | R&D~생산기술까지는 반도체形, 운영/서비스 계층이 별도로 크게 존재 |
