...
* 입력 이미지 처리: 좌측 상단에는 원본 28x28 이미지(숫자 '7')가 있습니다. 이 이미지가 'Flatten' 과정을 거쳐 784개의 픽셀 값을 가진 긴 벡터로 변환되는 과정을 보여줍니다.
* 첫 번째 은닉층: 784 벡터는 '은닉층 1: 128 뉴런'과 연결됩니다. 여기서 행렬 곱셈 연산이 수행되며, (784x1) 벡터가 (784x128) 행렬과 곱해져 (128x1) 결과를 생성합니다.
* 두 번째 은닉층: 생성된 128 벡터는 다시 (128x64) 행렬과 곱해져 '은닉층 2: 64 뉴런'의 입력이 됩니다. 최종적으로 (64x1) 크기의 벡터가 출력됩니다.
* 출력 및 분류: 마지막으로 64 벡터는 (64x10) 행렬과 곱해져 10개의 분류 값을 계산합니다. 가장 높은 확률을 가진 '7'이 최종 결과로 도출되며, 이를 '최종 10개의 분류' 그래프로 표현했습니다.
