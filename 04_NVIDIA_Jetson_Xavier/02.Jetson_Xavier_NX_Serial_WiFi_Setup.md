...
여기서는 `room410`의 신호가 `70`으로 비교적 강하게 검색되고 있다.

### 9.3 Wi-Fi 연결

```text
bready@bready-desktop:~$ sudo nmcli device wifi connect "room410" password "11111111"
...
`Device 'wlan0' successfully activated with '...'` 메시지가 출력되면 **`wlan0`이 `room410` AP에 성공적으로 연결된 것**이다.
